...
describe "Adding items to the cart", type: :feature do
  context "when adding a bundle to the cart" do
    ...
    context "when one of the variants is a pack" do
      specify "the cart displays the same quantity for part line item as that of the variant" do
        bundle = create(:product_in_stock, name: "Bundle", sku: "BUNDLE")
        keychain = create(
          :product_in_stock,
          name: "Keychain",
    ...
    context "when one of the bundle items has a variant (that is not user-selectable)" do
      specify "the cart includes that variant when listing items in the bundle" do
        bundle = create(:product_in_stock, name: "Bundle", sku: "BUNDLE")
        keychain = create(
          :product_in_stock,
          name: "Keychain",
      ...
      specify "the cart includes that variant when listing items in the bundle" do
        ...
        within("#cart-detail") do
          within("tbody tr:first-child") do
            ...
            expect(page).to have_css("input[value='1']")
            expect(page).to have_content("(1) Keychain (KEYCHAIN)")
            expect(page).to have_content("(1) Shirt (Size: Medium) (SHIRT-MEDIUM)")
           end
        end
      end
    end
